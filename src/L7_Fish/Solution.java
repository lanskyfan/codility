...
public class Solution {
    public int solution(int[] A, int[] B) {
        ...
        Stack<Integer> fish_weight = new Stack<Integer>();
        Stack<Integer> fish_direction = new Stack<Integer>();
        for (int i = 0; i < A.length; i++){
            if (fish_weight.isEmpty()){
                fish_weight.push(A[i]);
                fish_direction.push(B[i]);
                continue;
            }
            else if(B[i] == 1){
                fish_weight.push(A[i]);
            ...
            else if (fish_direction.peek() == 0){
                fish_weight.push(A[i]);
                fish_direction.push(B[i]);
            }
            else {
                while (true){
                    if (fish_direction.isEmpty() || fish_direction.peek() == 0){
                        fish_weight.push(A[i]);
                        fish_direction.push(B[i]);
                        break;
                    }
                    else if (fish_direction.peek() == 1 && fish_weight.peek() > A[i]){
                        break;
                    }
                    else if (fish_direction.peek() == 1 && fish_weight.peek() < A[i]){
                        fish_weight.pop();
