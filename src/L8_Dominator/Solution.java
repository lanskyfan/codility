package L8_Dominator;

/**
 * An array A consisting of N integers is given. The dominator of array A is the value that occurs in more
...
public class Solution {
    public int solution(int[] A) {
        ...
        Stack<Integer> leader_stack = new Stack<>();
        for (int x: A){
            if (leader_stack.isEmpty()){
                leader_stack.push(x);
            }
            else if (leader_stack.peek() != x){
                leader_stack.pop();
            }
            else {
                leader_stack.push(x);
        ...
        int candidate = leader_stack.peek();
        int count = 0;
        int position = 0;
